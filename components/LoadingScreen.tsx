...
// Expose replay via a global signal (simple event approach, no context needed)
const REPLAY_EVENT = 'mockingjay:replay-intro'

// Opt-10: Export a trigger so other components (e.g. a hidden button) can fire replay
...
export default function LoadingScreen() {
  ...
  const [key,     setKey]     = useState(0) // increment to remount overlay

  const handleDone = useCallback(() => setVisible(false), [])

  // Opt-10: listen for replay trigger
  useEffect(() => {
    const handler = () => {
      setKey((k: number) => k + 1)
      setVisible(true)
  ...
  return (
    <>
      ...
      {/* Opt-10: Hidden replay button — appears as a tiny corner icon after loading ends */}
      <AnimatePresence>
        {!visible && (
          <motion.button
            ...
            className="fixed bottom-5 right-5 z-40 group w-9 h-9 flex items-center justify-center border border-flame-orange/20 bg-ash-black/80 rounded-full hover:border-flame-orange/60 hover:bg-flame-orange/10 transition-all duration-300"
            aria-label="Replay intro"
          >
            {/* Replay icon */}
            <svg viewBox="0 0 18 18" className="w-4 h-4 text-flame-orange/40 group-hover:text-flame-orange transition-colors" fill="none" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round">
